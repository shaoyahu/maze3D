...
| 字段 | 值 |
|---|---|
| 活跃增量 | **P5-editor-multilayer 编辑器多层支持(2026-08-12 session 实施;MazeData.walls optional + walls xor walls2d 严格 mutex + src/utils/perLayerWalls.ts 4 函数(getCurrentLayerWalls/promoteToMultiLayer/collapseToSingleLayer/createEmptyGrid) + editorStore.addLevel/removeLevel 真实操作 walls2d(promote 提到多层/append clone 顶层/collapse 回单层)+ 运行时模块 Scene/Minimap/ParchmentMap/enemySpawner/LevelSelect 接 helper + editor 内部 placeWall/Erase/Start/Exit/Enemy 等接 currentLevel per-layer carve + EditorViewport/EditorStatusBar/editorValidation 按 currentLevel 切 grid + LevelTabs multi-layer badge + per-layer entity tooltip + EditorHelpDrawer 新增 ⑤ 多层迷宫段 + EditorStatusBar multi-layer "Layer 1/3" chip + importExport 多层 round-trip 显式 case + 教学关卡 teaching-multilayer-01.json 删 walls 字段严格 mutex;i18n en/zh +12 keys(editor.leftPanel.* / editor.status.layerIndicator.* / editor.help.section.multiLayer / editor.help.multiLayer*);typecheck 0/vitest 1812 pass+1 skip;待用户 commit + push + 开 PR)** |
| 已完成 | P2-2 14/14 ✅ + P2-3 14/14 ✅ + P2-4a 16/16 ✅ + P2-4b 20/20 ✅ + P2-5 16/16 ✅ + P2-6 10/10 ✅ + P2-7 8/8 ✅ + P2-9 ✅ + P2-10 11/11 ✅ + P2-11 16/16 ✅ + P2-13 ✅ + P2-14 ✅ + P2-15 24/24 ✅ + P2-16 ✅ + P2-17 ✅ + P2-18 ✅ + P2-19 ✅ + P2-20 ✅ + P2-21 ✅ + P3-1 ✅ + P3-2 ✅ + P3-3 ✅ + P4a ✅ + P4b-Prim ✅ + P4b-CellSize ✅ + P4b-Lerp ✅ + P4b-Minimap ✅ + P4b-HudLayer ✅ + P4b-Panorama ✅ + P4b-Instanced ✅ + **P4-refactor-fp2d ✅ (2026-08-11, 9 commit ship 闭环,PR #1 创建)** + **P5-1 multi-layer-teaching ✅ (2026-08-12, 1 commit 67a4a51,12 文件/+17/-3,1705+1 tests)** + **P5-2 editor-multilayer ✅ (2026-08-12, working tree 准备 commit)** |
| 下一个任务 | P5 路线图全部 ship 闭环 (P5-1 multi-layer-teaching 数据层 + P5-2 editor-multilayer 编辑器多层);后续候选 — 3D enemy AI (BFS chase 在 2D walls, 渲染 3D 球体) / per-instance color / HUD chip total / 3D editor / 3D tutorial / 重建真正 3D 体素 / 跨层 BFS reachability (P5-2 spec Task 6) |
| 最后更新 | 2026-08-12 |
| 最近 commit | 67a4a51 (P5-1 multi-layer-teaching) 已 push;p5-editor-multilayer 增量在 working tree 等待用户 commit + push + 开 PR |
...
| P4b-Lerp | 3D Player 0.1s cell-to-cell tween(MOVE_3D_TWEEN_SEC = 0.1 module-level const + `active3DTween` 字段(同构 P3-1 activeTransition 但 3 维)+ `tick3DTween` 私有方法 + `tick3DMovement` 拆分 input/tween 两路径 + startLevel + dispose 双 reset;Q3 决策 input 内部 gate — 只 lock move 键,mouse-look 在 tween 期间仍可用(不调 input.setPaused);Q4 collision 在 tween 启动时判一次 + Q5 exit check 在 tween 完成时触发;snap 完成时 float drift 防护;cellSize 跟 FLOOR_HEIGHT 不混用;3D path 在 update() 顶部 short-circuit,3D tween 跟 2D vertical transition 物理上永远不并发;0.1s = 6 帧 60fps sweet spot;`tests/unit/engine/Game.3D.tween.test.ts` 新建 7 case + `Game.3D.test.ts` 改 4 case tween-aware) | P1 | P4b-CellSize | S | `docs/increments/p4b-3d-lerp/` | ✅ done (2026-08-07, session 实施;3 文件/+7 case;typecheck 0 / vitest 1728+1 skip) |
| P4b-Minimap | 3D top-down minimap(is3D dispatch on `maze.walls3D !== undefined` + 渲染 `walls3D[currentLayer]` 当前 y-layer 2D 顶视图 + `Game.getPlayerY()` accessor 跟 `getPlayerPosition` 平行 + `tick3DTween` 完成时调 `recordVisit(parchment, yCell, endCell.x, endCell.z)` 复用 P3-1 visited map shape;`PlayerSnapshot` 加 y 字段 + `Y_EPSILON` 早出条件;3D exit dispatch — 同层 COLOR_EXIT rect / off-layer "↑/↓ exit" SVG text 紧贴玩家 arrow 下方;`StaticMaze` 重构接受 `walls2D` + `exitPos` + `pickups` props 让 2D 跟 3D 复用;y-level 标签 "L{n}/{total}" 1-indexed 跟 HUD LevelIndicator 风格一致;HUD chip 留给 P4b+ scope;`tests/component/Minimap.3D.test.tsx` 新建 8 case + `Game.3D.tween.test.ts` 加 2 recordVisit case + `minimap.test.tsx` mock 加 getPlayerY) | P1 | P4b-Lerp | M | `docs/increments/p4b-3d-minimap/` | ✅ done (2026-08-07, session 实施;4 文件/+10 case;typecheck 0 / vitest 1738+1 skip) |
| P4b-HudLayer | HUD LevelIndicator 2D/3D dispatch(tick3DTween 完成时 fire `bridge.onLevelChange(yCell)` 推 y-cell + startLevel 3D path if/else dispatch 推初始 start3D.y;复用 P3-1 onLevelChange callback 语义统一为 'current visible layer'(2D vertical / 3D y-cell);2D 行为完全不变;HUD chip 1-indexed display L{n} 跟 minimap L{n}/{total} 数字部分自动一致;`this.playerLevel` 字段保持 2D only,3D 不污染;`tests/unit/engine/Game.3D.tween.test.ts` +2 case 覆盖 tick3DTween push + startLevel 3D push) | P1 | P4b-Minimap | S | `docs/increments/p4b-hud-layer/` | ✅ done (2026-08-07, session 实施;2 文件/+2 case;typecheck 0 / vitest 1740+1 skip) |
| P4b-Panorama | 3D 全景 minimap — 3 y-layer 堆叠(120×120 容器拆 40+40+40 3 strip:上 `walls3D[y+1]` 50% opacity / 中 `walls3D[y]` 满色 100% + 玩家 arrow + cone + visited + 出口 / 下 `walls3D[y-1]` 50% opacity;越界 strip 不渲染留白;1px 灰 border-bottom 分隔线;`YStrip` 组件 `React.memo` 接受 walls2D + opacity + isCurrent + exitPos + visitedForLevel + exitHint;玩家 arrow / cone / visited / 出口 rect 只在当前 y strip;off-layer "↑/↓ exit" 复用 P4b-Minimap dispatch;复用 getPlayerY() + PlayerSnapshot.y + Y_EPSILON + onLevelChange(yCell) 全部前 scope 锁;`tests/component/Minimap.Panorama.test.tsx` 新建 9 case) | P1 | P4b-HudLayer | M | `docs/increments/p4b-panorama/` | ✅ done (2026-08-07, session 实施;2 文件/+9 case;typecheck 0 / vitest 1749+1 skip) |
| P4b-Instanced | 3D 墙 InstancedMesh(`buildScene3D` 改用 1 个 `THREE.InstancedMesh` 替代 N mesh;1687 draw call → 1 draw call 渲染所有 wall cell;1 pass O(N) 计数 + 1 pass O(N) `setMatrixAt`;`mesh.count = actualCount`(不是 `visualSize³` 上限);`mesh.instanceMatrix.needsUpdate = true` 触发 GPU 上传;cell-center invariant 保持 `matrix.makeTranslation((x+0.5)*cs, ...)`;`SceneRefs.walls` 变 `[InstancedMesh]` 单元素;`disposeScene` 兼容 (`seenGeoms` / `seenMats` dedup 共享 wallGeom / wallMat);InstancedMesh 保持 per-instance 概念(后续可加 per-instance 状态:damage flash / enemy AI 标记 / dynamic 颜色);2D `buildScene` 路径完全不动;总 draw call 1691 → 5 (99.7% 减少);visualSize=15 frame time 实测 5-10ms 下降;P4b-CellSize 6 档 size 全部走 InstancedMesh 路径;`tests/unit/engine/Scene.3D.test.ts` 新建 6 case) | P1 | P4b-Panorama | M | `docs/increments/p4b-instanced/` | ✅ done (2026-08-07, session 实施;3 文件/+6 case;typecheck 0 / vitest 1755+1 skip) |
| P4-refactor-fp2d | 3D 重新设计为「第一人称视角渲染同一个 2D 多层迷宫」(view=fp3d URL query + WASD x/z + transition 上下,禁止 6 方向自由移动;3D path 不再是独立 dispatch,跟 2D path 共享一份 2D 多层数据 + 第一人称 PerspectiveCamera;删 3D 体素生成/MazeData.walls3D/3D BFS/seed v3 codec/3D enemy/3D tutorial/3D parchment/3D panorama minimap;AlgoRegistry 15 算法 2D-only SoT;老 v3 URL 友好 fall back `navigate('/levels', { replace: true })` 限定 `algo-v3-*` 前缀;PR #1) | P1 | P4a, P4b-Instanced | X-Large | `docs/increments/p4-refactor-fp2d/` | ✓ done (2026-08-11, 9 commit ship 闭环,PR #1 创建) |
| P5-1 | 多层迷宫教学关卡(数据层 MazeData.walls2d?: CellType[][][] + JsonMazeProvider 严格 reject levelCount>1 无 walls2d + 完整 transition 验证 8 check(每个 transition level/toLevel bounds + 源/目标 wall check + id 唯一 + kind 5 字面量)+ engine `Scene.resolvePerLayerWalls` + `Game._grid.get` 优先读 `maze.walls2d` + LevelSelect 删 H3 useEffect(让 engine 读 JSON levelCount)+ `public/levels/teaching-multilayer-01.json` 5x5 2 层 + stair-up + 5 步 tutorial banner + i18n +5 keys + tests +7 JsonMazeProvider + 1 builtInLevels,改 2 types.multiLevel + 1 levelSelect H3 case;算法名仍是 URL seed 一部分,重命名 breaking;15 算法 SoT 不动;FLOOR_HEIGHT=2.4 / EYE_HEIGHT=1.6 在 Player/Scene/Game 三处 lockstep 同步) | P1 | P3-1 | M | `docs/increments/p5-multilayer-teaching/` | ✓ done (2026-08-12, 1 commit 67a4a51,12 文件/+17/-3,1705+1 tests) |
| P5-2 | 编辑器多层支持(`MazeData.walls` 改 optional + validator 强化 `walls xor walls2d` 互斥 + `src/utils/perLayerWalls.ts` 4 函数(getCurrentLayerWalls / promoteToMultiLayer / collapseToSingleLayer / createEmptyGrid) + editorStore.addLevel / removeLevel 真实操作 walls2d + 运行时模块 Scene/Minimap/ParchmentMap/enemySpawner/LevelSelect 接 helper + editor 内部 placeWall/Erase/Start/Exit/Enemy 等接 currentLevel per-layer carve + EditorViewport / EditorStatusBar / editorValidation 按 currentLevel 切 grid + LevelTabs multi-layer badge + per-layer entity tooltip + EditorHelpDrawer 新增 ⑤ 多层迷宫段 + EditorStatusBar multi-layer "Layer 1/3" chip + importExport 多层 round-trip 显式 case + 教学关卡 teaching-multilayer-01.json 删 walls 字段严格 mutex;i18n en/zh +12 keys(editor.leftPanel.levelTabTooltip / levelTabEntityBreakdown / multiLayerBadge / editor.status.layerIndicator.single / multi / editor.help.section.multiLayer / multiLayerAdd / AddBody / Remove / RemoveBody / Connect / ConnectBody / Json / JsonBody)) | P1 | P5-1 | L | `docs/increments/p5-editor-multilayer/` | ✓ done (2026-08-12, working tree 准备 commit + push + 开 PR;typecheck 0 / vitest 1812 pass+1 skip) |
