...
| 已完成 | 2 / 13 |
| 下一个任务 | **#4 `App.tsx` useEffect 同步 `settingsStore.darkMode → data-theme`** |
| 最后更新 | 2026-06-08 |
...
| 9 | `Rules.ts` 实现 useItem handler（无锁门 = slot 高亮闪烁） | S | [ ] |
| 10 | `Pickup` mesh 按 type 选颜色：time 金黄 / health 红 / key 蓝 | S | [ ] |
| 11 | `InventoryBar` 加数字键提示 + slot 高亮激活态 | S | [ ] |
