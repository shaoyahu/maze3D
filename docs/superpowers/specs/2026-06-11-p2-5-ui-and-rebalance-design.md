# P2-5: 界面改版 + 存活模式重平衡 — 设计 (Brainstorming)

**Slug**: `p2-5-ui-and-rebalance`
**Status**: draft (脑暴输出,等待用户审阅)
**Date**: 2026-06-11
**对应路线图项**: P2-N (新增;取代 "P2-N/A: 等待用户决策" 占位)
**依赖**: —
**复杂度**: Large
**相关文件**: `src/ui/MainMenu.tsx`, `src/ui/LevelSelect.tsx`, `src/ui/components/Button.tsx`, `src/ui/components/EnemyCounter.tsx`, `src/store/gameStore.ts`, `src/engine/Game.ts`, `src/maze/AlgorithmMazeProvider.ts`, `src/maze/types.ts`, `src/maze/enemySpawner.ts`, `theme.css` (或等效 CSS), `docs/increments/_template/roadmap.md`

> 本文档是脑暴阶段的产出。批准后,writing-plans 技能会按照现有增量模板把它翻译成 `docs/increments/p2-5-ui-and-rebalance/{spec.md, plan.md, review.md}`。
