...
### P4 refactor-fp2d — locked contracts (2026-08-11)
...
- 1694 tests pass (1 个历史 skip,跟 refactor 无关),零 2D 回归
- **老 `algo-v3-…` URL 友好 fall back 到 /levels**(P4 refactor spec §8/§10/§11 决策 + Bug #2 修):`App.tsx` `useEffect` 检测 `seed?.startsWith('algo-v3-')` + `parsed.error === 'bad-seed'` 时 `console.warn('[P4 refactor-fp2d] v3 (3D voxel) seed URL detected — falling back to /levels. …')` + `navigate('/levels', { replace: true })`。**严格限定 v3 prefix**:`?seed=not-a-real-seed` 这类 generic bad-seed 仍走红色错误页(genuine user mistake)。headless Browser pointer lock 失败属 expected(toast 错误处理路径已工作)。
