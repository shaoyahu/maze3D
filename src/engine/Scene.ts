...
import type { CellType, KeyColor, MazeData, VerticalTransition, ViewMode } from '../maze/types';
...
export function buildScene(maze: MazeData, darkMode =false, view: ViewMode = '2d'): SceneRefs {
  // P4 refactor-fp2d: the 3D voxel `walls3D !== undefined`
  // dispatch is removed. The 3D mode the user now sees is a
  // first-person perspective camera rendering the SAME 2D
  // multi-layer data the 2D top-down path consumes, so there
  // is no per-scene-build branch on the maze shape — both
  // views share the same mesh tree. The `view` parameter
  // currently only changes one thing: the player marker is
  // hidden in first-person mode (the player can see their
  // own feet / hands, no need for an on-floor ring). Every
  // other mesh (walls, floor, ceiling, exit, pickups,
  // enemies, traps, doors, transitions) is identical
  // because the 2D "top-down" rendering was already
  // PerspectiveCamera + 3D mesh — the only difference
  // between the two views is the camera position +
  // orientation, which is owned by Camera.ts / Game.ts,
  // not Scene.ts.
  ...
  //
  // P4 refactor-fp2d: the marker is hidden in first-person mode
  // because the player is looking out from the marker's position
  // and would never see the ring under their feet anyway. The
  // SceneRefs still carries the mesh (the engine writes to
  // `playerMarker.position` every frame regardless of view, so
  // the dispose path stays uniform), but `visible = false` keeps
  // it out of the render.
...
// ---------------------------------------------------------------------------
// P4 refactor-fp2d: the 3D voxel `buildScene3D` function is
// removed. The 3D mode the user now sees is a first-person
// perspective camera rendering the SAME 2D multi-layer data
// the 2D top-down path consumes, so the scene graph is
// shared between the two views — `buildScene` is the only
// scene builder, parameterized by `view: ViewMode` to flip
// the player-marker visibility. The 3D voxel-specific
// assets (cuboid-per-cell walls, no floor / ceiling, no
// entities, no warning rings) are gone in lockstep with
// the `walls3D` / `start3D` / `exit3D` data fields removed
// in `maze/types.ts`; the engine now has a single
// movement + scene path for every level regardless of view.
// ---------------------------------------------------------------------------
