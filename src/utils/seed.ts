...
// P4 refactor-fp2d: the v3 codec (3D voxel) is removed. The 3D
// mode is now a "first-person view of 2D multi-layer" rendering
// (P4 refactor spec), so 3D voxel seeds (algo-v3-…) are no
// longer produced. Old v3 URLs fall through to the bad-seed
// error path; a future codec bump (v4) is the right migration
// when this matters. The v1 / v2 codecs (single-layer / multi-
// layer 2D) are unchanged.
...
// P4 refactor-fp2d: encodeSeedV3 + SeedV3 + the v3 branch in
// decodeSeed are removed. The 3D voxel codec (algo-v3-…) is no
// longer produced; 3D mode is now "first-person view of 2D
// multi-layer" (P4 refactor spec) and uses the v2 codec (multi-
// layer 2D) + a new `view=fp3d` URL query.
