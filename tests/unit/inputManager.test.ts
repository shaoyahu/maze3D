...
describe('InputManager', () => {
  ...
  // P4 refactor-fp2d: the 3D 6-neighbor `getMove3D()` test
  // block is removed. The 3D voxel movement model (Space = +y,
  // KeyC = -y, free 6-direction walk) is gone; the new first-
  // person 3D view uses the SAME 2D `getMove()` (WASD on x/z)
  // + `getLadderRequest()` (Space/C, 2D ladder only) for
  // layer transitions. The locked contract is "3D mode ≠
  // 6-direction free movement, 3D mode = first-person view of
  // 2D multi-layer" — so the up/down axis is reachable ONLY
  // through transition tiles, not through free keypresses.
  // The P3-1d ladder test below covers the remaining Space/C
  // bindings; the W/A/S/D path is the historical 2D `getMove()`
  // test block above.

  // P3-1d: 2D ladder request. The bindings use Space (up) and
  // KeyC (down) — the same keys the old 3D `getMove3D` used
  // for its y-axis, so the muscle memory is consistent across
  // the 2D top-down and first-person 3D views. The actual
  // trigger only fires when the player is standing on a
  // ladder cell; this test pins the InputManager half (the
  // keypress → flag plumbing).
