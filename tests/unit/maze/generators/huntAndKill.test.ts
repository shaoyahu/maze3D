...
import { generateHuntAndKill } from '../../../../src/maze/generators/huntAndKill';
...
describe('generateHuntAndKill', () => {
  it('returns a 2D array of 0/1 cells of the right shape', () => {
    const walls = generateHuntAndKill(15, mulberry32(1));
  ...
  it('is deterministic for the same seed', () => {
    const a = generateHuntAndKill(15, rngFromHexSeed('0123456789abcdef'));
    const b = generateHuntAndKill(15, rngFromHexSeed('0123456789abcdef'));
    expect(a).toEqual(b);
  });

  it('produces different outputs for different seeds', () => {
    const a = generateHuntAndKill(15, rngFromHexSeed('0000000000000001'));
    const b = generateHuntAndKill(15, rngFromHexSeed('0000000000000002'));
    expect(a).not.toEqual(b);
  });

  it('produces a fully connected maze (start reaches exit) at 15×15', () => {
    const walls = generateHuntAndKill(15, mulberry32(42));
    expect(isReachable(walls, { x: 0, z: 0 }, exitCell(15))).toBe(true);
  });

  it('produces a fully connected maze at 30×30', () => {
    const walls = generateHuntAndKill(30, mulberry32(42));
    expect(isReachable(walls, { x: 0, z: 0 }, exitCell(30))).toBe(true);
  });

  it('produces a fully connected maze at 50×50', () => {
    const walls = generateHuntAndKill(50, mulberry32(42));
    expect(isReachable(walls, { x: 0, z: 0 }, exitCell(50))).toBe(true);
  });

  it('50×50 generation completes in under 500ms', () => {
    const t0 = performance.now();
    generateHuntAndKill(50, mulberry32(7));
